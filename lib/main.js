...
const ui = {
    ...
    map: {
        ...
        onIdle: () => {
            ...
            ui.dialog.preference.updateLocation(center.lng, center.lat, zoom);
        ...
        setLocation: () => {
            ...
            if (isNaN(lon) || lon < -180 || lon > 180) {
                dialogPref.location.lon.value = value.location.lon;
                inputError = true;
            }
            if (isNaN(lat) || lat < -90 || lat > 90) {
                dialogPref.location.lat.value = value.location.lat;
                inputError = true;
            }
            if (isNaN(zoom) || zoom < 0 || zoom > 20) {
                dialogPref.location.zoom.value = value.location.zoom;
                inputError = true;
            }
            if (inputError) {
                return;
